...
| --- | --- | --- | --- |
| Unit | Parser behaviours, logging helpers | `packages/tokenparser-api-shared/src/**` with `npm run test:unit` | Required in shared workspace |
...
## 7. Open Improvements
1. Formalise parser unit tests under `packages/tokenparser-api-shared/tests` (now available via `npm run test:unit`).
