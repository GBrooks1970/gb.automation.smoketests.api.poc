...
export class TokenDynamicStringParser {
  ...
  public static parseAndGenerate(token: string): string {
    ...
    if (!match || !match.groups) {
      throw new Error(
        `${TokenDynamicStringParser.name} : ${TokenDynamicStringParser.errorInvalidStringTokenFormat} : ${token}`,
      );
    }
    ...
    if (charSet === "") {
      throw new Error(
        `${TokenDynamicStringParser.name} : No valid character types found in token: ${token}`,
      );
    }
    ...
    for (let line = 0; line < linesNum; line++) {
      ...
      logger.debug(
        `TOKEN ${token} : Generated string LINE[${line}]'${CommonUtils.toJSONString(lineResult)}'`,
      );
