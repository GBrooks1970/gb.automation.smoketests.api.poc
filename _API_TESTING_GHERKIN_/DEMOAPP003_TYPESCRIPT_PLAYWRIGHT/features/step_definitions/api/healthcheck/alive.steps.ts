...
Given<CustomWorld>("the API is available", async function (this: CustomWorld) {
  await this.actor.attemptsTo(SendGetRequest.to("/alive"));
  const status = await this.actor.answer(ResponseStatus.code());
  expect(status).toBe(200);
});

When<CustomWorld>("a GET request is made to the Alive Endpoint", async function (this: CustomWorld) {
  await this.actor.attemptsTo(SendGetRequest.to("/alive"));
});

Then<CustomWorld>("the API response should return a status code of 200", async function (this: CustomWorld) {
  const status = await this.actor.answer(ResponseStatus.code());
  expect(status).toBe(200);
});
